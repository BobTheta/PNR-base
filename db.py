...
#             configs["prefix"] = model.global_var.PREFIX
#             configs["url"] = model.global_var.SERVER_URL
#             self.db = HttpDB(**configs)
    ...
    def get_instance_properties(self, entity_id):
        ...
        if model.global_var.CACHE_FLAG:
            if entity_id not in model.global_var.WHOLE_INFO_RESULT:
                sq = 'select * from <%s> where {<%sinstance/%s> ?p ?o}'%(model.global_var.GRAPH, model.global_var.PREFIX,entity_id)
                result_set = self.db.query2(sq)
                if result_set:
                    result = {}
                    for p, o in result_set:
                        result[p] = result.get(p,[]) + [o]                
                    model.global_var.WHOLE_INFO_RESULT[entity_id] = result            
            return model.global_var.WHOLE_INFO_RESULT.get(entity_id,{})
        else :
            sq = 'select * from <%s> where {<%sinstance/%s> ?p ?o}'%(model.global_var.GRAPH, model.global_var.PREFIX,entity_id)
            result_set = self.db.query2(sq)
            result = {}
...
#                     print(p2o)
                    continue
                if obj.startswith(model.global_var.PREFIX):
                    if p != 'actor_list' :
        ...
        if model.global_var.STATISTIC_FLAG:
            ...
            model.global_var.ACCESS_DATABASE_TIME += duration
        ...
        def deep_instance(d, p):
            ...
            if p in d.keys():
                for e in d[p]:#注意value是list形式的
                    if e.startswith(model.global_var.PREFIX):
                        i = e.split("/")[-1]
                        l = self.get_label(i)
                        if not l:
                            return s
...
#                         print("debug :" + e)
    ...
    def get_abstract(self, entity_id):
        #sq = 'select * from <%s> where {<%sinstance/%s> <%scommon/summary> ?o }'%(model.global_var.GRAPH, model.global_var.PREFIX, entity_id, model.global_var.PREFIX)
        sq = 'select * from <%s> where {<%sinstance/%s> ?p ?o}'%(model.global_var.GRAPH, model.global_var.PREFIX,entity_id)
        result_set = self.db.query2(sq)
        for p, o in result_set:
    ...
    def get_label_uri(self, uri):
        ...
        #sq = 'select * from <%s> where {<%sinstance/%s> <%scommon/summary> ?o }'%(model.global_var.GRAPH, model.global_var.PREFIX, entity_id, model.global_var.PREFIX)
        if model.global_var.CACHE_FLAG:
            if uri not in model.global_var.LABEL_DICT :
                sq = 'select * from <%s> where {<%s> ?p ?o}'%(model.global_var.GRAPH, uri)
                result_set = self.db.query2(sq)
                for p, o in result_set:
                    if p.endswith('label/zh'):
                        model.global_var.LABEL_DICT[uri] = o
                        break
            return model.global_var.LABEL_DICT.get(uri)
        else:
            sq = 'select * from <%s> where {<%s> ?p ?o}'%(model.global_var.GRAPH, uri)
            result_set = self.db.query2(sq)
            for p, o in result_set:
            ...
            if uri not in model.global_var.LABEL_DICT :
                ...
                for p, o in result_set:
                    if p.endswith('actor_id'):
                        ...
                        return label
                    elif p.endswith('actor_name'):
                        label = o
                return label 
            else: return model.global_var.LABEL_DICT[uri]
        else:
            sq = 'select * from <%s> where {<%s> ?p ?o}'%(model.global_var.GRAPH, uri)
            result_set = self.db.query2(sq)
            for p, o in result_set:
...
#         sq = 'select * from <%s> where {<%sinstance/%s> ?p ?o}'%(model.global_var.GRAPH, model.global_var.PREFIX,entity_id)
        sq = 'select * from <%s> where {<%sconcept/%s> ?p ?o}'%(model.global_var.GRAPH, model.global_var.PREFIX,entity_id)
        result_set = self.db.query2(sq)
        for p, o in result_set:
            if p.endswith('label/zh'):
                return o
    ...
    def create_littleentity(self, entity_id):
        ...
        entity["uri"] = model.global_var.PREFIX+entity_id
...
#         if d.has_key("firstimage"):
    ...
    def get_entity_label(self, entity_id):
        #sq = 'select * from <%s> where { <%s> <%sobject/label/zh> ?o }'%(model.global_var.GRAPH, entity_id, model.global_var.PREFIX)
        sq = 'select * from <%s> where {<%s> ?p ?o}'%(model.global_var.GRAPH, entity_id)
        result_set = self.db.query2(sq)
        for p, o in result_set:
            if p.endswith('label/zh'):
                return o
        ...
        def get_baseinfo(entity_id,predictmap):
            ...
            for p, o in result_set:
                ...
                if p in  predictmap['objectType']:
                    ...
                    if o.startswith(model.global_var.PREFIX):
            ...
            for pname in objects:
                if pname == u'演员表':
                    ...
                    for uid in objects[u'演员表']:
                        sq = 'select * from <%s> where {<%s> ?p ?o}'%(model.global_var.GRAPH,uid)
                        result_set = self.db.query2(sq)
...
if __name__ == "__main__":
    #configs = ConfigTool.parse_config("./config/db.cfg","MovieKB")
    mkb = MovieKB()
    result = mkb.get_whole_info_label('dn10037060')
    print(len(result))
